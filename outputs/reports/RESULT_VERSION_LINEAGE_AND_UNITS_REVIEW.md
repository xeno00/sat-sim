...
| normalized_standard_benchmark_cards_v1 | outputs/standard_benchmark_cards | codex/normalized-benchmark-adapters-v1 | pending | scripts/run_standard_benchmark_cards.py --run --case-id std_nu3_ns10_fullmesh_los_clock1us_seed0 --output-root outputs/standard_benchmark_cards --force | registered_pipeline_specs_mixed | per-card | per-card | per-card | benchmark_card | per-card; package_native_c7 uses truth only for offline metrics and unavailable adapters do not execute | per-card | std_nu3_ns10_fullmesh_los_clock1us_seed0 | 1105.15 | 4289.81 | 4287.92 | 937.214 | 3882.06 | 3879.79 | std_nu3_ns10_fullmesh_los_clock1us_seed0 | available_partial_package_native_only | 1105.15 | 4289.81 | 4287.92 | 937.214 | 3882.06 | 3879.79 | std_nu3_ns4_fullmesh_los_clock1us_seed0 | secondary_low_satellite_stress_case | not_applicable | unknown | unknown | unknown | unknown | unknown | unknown | first normalized benchmark-card run; package_native_c7 executes while other adapters are explicit missing cards | use_for_human_review | use_for_debugging_only | Non-final one-row benchmark card; not manuscript evidence. |
...
Next required diagnostic: add safe executable adapters for `controlled_migration_step_b_lm_only` and `legacy_surgical_prior_region`, then rerun the same normalized benchmark-card command for `std_nu3_ns10_fullmesh_los_clock1us_seed0`.
...
| normalized_standard_benchmark_cards_v1 | use_for_debugging_only | Use only to validate the adapter/schema interface. | package_native_c7 executes on the primary case; non-package-native adapters remain unavailable and no manuscript claim is supported. |
...
- Next diagnostic action: Add the controlled Step B and legacy-surgical adapters to the normalized benchmark-card runner, then rerun the same std_nu3_ns10 primary card before any manuscript evidence claim.
